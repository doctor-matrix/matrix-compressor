...
# trainset = torchvision.datasets.CIFAR10(root='./artifacts/datasets', train=True, download=True, transform=transform)
...
test_embedding_checkpoint = torch.load(
    repo_basepath / "artifacts/custom_data/knn/mnet_v2/cifar10/test-embeddings-labels.pt",
    map_location=map_location,
)
...
# Train a nearest neighbors classifier using the naively quantized embeddings
knn_lplr = FaissKNeighborsGPU(n_neighbors=3)
...
test_lplr_predictions = knn_lplr.predict()
