...
# Extract embeddings of the training set
test_embeddings, test_labels = extract_embeddings(trainloader, model)

logger.info(f"test_embeddings.shape: {test_embeddings.shape}")
logger.info(f"type(test_embeddings): {type(test_embeddings)}")
logger.info(f"test_labels.shape: {test_labels.shape}")

torch.save(
    {"test_embeddings": test_embeddings, "test_labels": test_labels},
    out_path / "cifar10-test-embeddings.pt",
)